...
ComFun.isPhoneNumber = function(phone)
    return string.match(phone,"[1][3,4,5,7,8]%d%d%d%d%d%d%d%d%d") == phone
end
...
ComFun.getPassword = function()
    ...
    if ComFun.isPassword(password) then
        return password
    end
    return "0"
end
...
ComFun.setPhone = function(str)
    if ComFun.isPhoneNumber(str) then
...
-- 设置密码
ComFun.setPassword = function(str)
    if ComFun.isPassword(str) then
...
--加密
ComFun.encrypt = function(str)
    return crypto.encryptXXTEA(str , cryptokey)
end

-- 解密
ComFun.decrypt = function(str)
    return crypto.decryptXXTEA(str , cryptokey)
end
