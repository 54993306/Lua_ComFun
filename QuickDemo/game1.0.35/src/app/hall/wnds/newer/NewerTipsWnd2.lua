...
function NewerTipsWnd2:onInit()
    ...
    local pan_content = ccui.Helper:seekWidgetByName(self.m_pWidget,"pan_content")
    local panSize = pan_content:getContentSize()
    local nextSize = self.btn_next1:getContentSize()
    self.btn_next1:getLayoutParameter():setMargin({ left = 0, right = 200, top = 0, bottom = 69})
    local next_img = ccui.Helper:seekWidgetByName(self.btn_next1,"img");
    next_img:loadTexture("hall/newer/img_yes.png")
    --上一页                           
...
function NewerTipsWnd2:onClickButton(pWidget, EventType)
    if EventType == ccui.TouchEventType.ended then
        ...
        if pWidget == self.btn_close or pWidget == self.btn_next or pWidget == self.btn_next1 then
