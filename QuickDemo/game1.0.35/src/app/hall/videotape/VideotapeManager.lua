...
function VideotapeManager:startScheduleInfo()
    ...
    self.playHandle = scheduler.scheduleGlobal( function(dt)
        ...
        if self.startTime >= self.intervalTime then
            ...
            --  设置快进允许标志
            self.startTime = 0
            if self.count < #playbackInfo then
                local info = { }
...
function VideotapeManager:reSponseInfo(fileName)
    ...
    local jsonFile = cc.FileUtils:getInstance():fullPathForFilename(fileName)
    -- 解析完成后移除文件
    local removePath = CACHEDIR .. fileName;
    -- cc.FileUtils:getInstance():removeFile(removePath)
    -- 获取文件数据
    local levelFileStr = cc.HelperFunc:getFileData(jsonFile)
    ...
    for i = 1, #levelFileData.messages do
        ...
        if jsonCont.recUserId == myUserid then
            if levelFileData.messages[i].code == kSubCodeType.gameOver
                ...
                -- 过滤听牌消息
                if tempInfo.subcode == kSubCodeType.reqPlayCard then
    ...
    -- dump(myInfo)
    -- dump(startInfo)
    local roomidOne = self:getJsonFileName(startInfo[myUserid].content.plID, "-", false)
    local roomid = self:getJsonFileName(roomidOne, "-", true)
