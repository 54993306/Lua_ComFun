...
local FileLogUrl = "http://client-log-gather.stevengame.com/upload/logUpload"
...
function FileLog.init(deviceCacheDir)
    ...
    testLog("cacheDir", cacheDir)
    lfs.mkdir(cacheDir)

    compressedDir = cacheDir .. "compressed/"
    testLog("compressedDir", compressedDir)
    lfs.mkdir(compressedDir)
...
function FileLog.uploadAllLog()
    Log.i("FileLog.uploadAllLog() firstUpLoad", firstUpLoad)
    Log.i("compressedDir", compressedDir)
    if not firstUpLoad or not compressedDir then return end
    firstUpLoad = false
...
--     if fileLogCfg.white == 0 or fileLogCfg.whiteUids[userid] then return true end
-- end
...
function FileLog.onPostLog(filePath, svrFilePath, callback)
    testLog("FileLog.onPostLog filePath: " .. filePath)
    testLog("FileLog.onPostLog svrFilePath: " .. svrFilePath)
    network.uploadFile(
        function(evt)
...
function FileLog.initFile(filePath)
    if type(filePath) ~= "string" then
        ...
        FileLog.saveToFile(GC_GameName .. os.date("_Log_Start_%Y%m%d_%H%M%S\n\n"), filePath, "w+b")
    end
end
